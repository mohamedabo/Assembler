...
as textual strings of 1's and 0's."""
    ...
    def gen_a(self, addr):
        """Generates the A instruction"""
        return '0' + self._bits(addr).zfill(15);

    def gen_c(self, dest, comp, jump):
        """Generates the C instruction"""
        return '111' + self.comp(comp) + self.dest(dest) + self.jump(jump)

    _dest_codes = ['', 'M', 'D', 'MD', 'A', 'AM', 'AD', 'AMD'] # destination codes
    def dest(self, d):
        """Returns the destination bits"""
    ...
    _comp_codes = { '0':'0101010',  '1':'0111111',  '-1':'0111010', 'D':'0001100',
                    ...
                    'D&M':'1000000', 'D|M':'1010101' } # the binary dictionary for instructions
    def comp(self, c):
        """Returns the comp codes in C instruction"""
        return self._comp_codes[c]

    _jump_codes = ['', 'JGT', 'JEQ', 'JGE', 'JLT', 'JNE', 'JLE', 'JMP'] # the jump codes
    def jump(self, j):
        """Returns the jump bits"""
        return self._bits(self._jump_codes.index(j)).zfill(3)

    def _bits(self, n):
        """Returns the instructions as 0 and 1"""
        return bin(int(n))[2:]
